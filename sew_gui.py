...
class SewGUI(wx.Frame):
    def __init__(self, *args, **kw):
        ...
        lbl.SetFont(lbl.GetFont().Bold())
        newline.Add(lbl,flag=wx.ALIGN_TOP)

        unit_opts = ["Inches","Centimetres"]     
        self.unit_box = wx.RadioBox(pnl,label='Units', choices=unit_opts, style = wx.RA_SPECIFY_COLS)
        newline.Add(self.unit_box,flag=wx.LEFT, border=50)
        vert_sizer.Add(newline, flag=wx.EXPAND|wx.TOP|wx.LEFT, border=5)
    ...
    def on_go_pressed(self,event):
        ...
        # margins
        self.tiler.set_margin(self.txt_to_pixels(self.margin_txt.GetValue()))

        # trim
        trim = [0.0]*4
        trim[0] = self.txt_to_pixels(self.left_trim_txt.GetValue())
        trim[1] = self.txt_to_pixels(self.right_trim_txt.GetValue())
        trim[2] = self.txt_to_pixels(self.top_trim_txt.GetValue())
        trim[3] = self.txt_to_pixels(self.bottom_trim_txt.GetValue())
    ...
    def txt_to_pixels(self,txt):
        ...
        if self.unit_box.GetSelection() == 0:
            return txtnum*72
        
        else:
            return txtnum*72/2.54
    ...
    def on_output(self, event):
        with wx.FileDialog(self, "Save output as", defaultDir=self.working_dir,
            ...
            try:
                ...
                wx.LogError("Cannot output to file '%s'." % pathname)
